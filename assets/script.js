...
let videojuegos = [
  {
    ...
    nombre: "FIFA 2023",
    descripcion:
      "La innovadora version de FIFA te sorprenderá con su nueva jugabilidad.",
    descripcionLarga:
      "<p>FIFA 23 es un videojuego de simulación de fútbol publicado por Electronic Arts.</p> <p> Es la trigésima entrega de la serie FIFA desarrollada por EA Sports, y la última entrega bajo el estandarte de FIFA, y lanzada en todo el mundo el 30 de septiembre de 2022 para PC, Nintendo Switch, PlayStation 4, PlayStation 5, Xbox One, Xbox Series X. /S y Google Stadia.</p><p>  Sin embargo, los jugadores que reservaron la Ultimate Edition recibieron tres días de acceso anticipado y pudieron jugar el juego a partir del 27 de septiembre.</p>",
    precio: 8000,
  ...
  },
  {
    ...
    nombre: "Far Cry 5",
    descripcion:
      "La quinta edición del Far Cry. Un videojuego de guerra en primera persona.",
    descripcionLarga:
...
let catalogoMostrado = false,
  guerraMostrado = false,
  aventuraMostrado = false,
  DeportesMostrado = false;
...
const agregarAlCarrito = (id) => {
  ...
  if (existe) {
    const videojuegoCant = carrito.find(
...
const crearCarritoModal = () => {
  ...
  carrito.forEach((videojuego) => {
    ...
    contenedorCatalogoModalCarrito.setAttribute(
      "id",
      "contenedorCatalogoModalCarrito"
    );
    contenedorCatalogoModalCarrito.appendChild(contenedorVideojuegomodal);
    contenedorVideojuegomodal.setAttribute(
      "class",
    ...
    contenedorTituloPrecio.setAttribute(
      "class",
    ...
    contenedorbotonesmodalCarrito.setAttribute(
      "class",
    ...
    precioVideojuegomodal.textContent =
      "SUBTOTAL:$" + videojuego.precio * videojuego.cantidad;
  ...
  botonFinalizarCompra.addEventListener("click", function irFormFinalizar() {
    if (carrito.length > 0) {
      ...
      sinProductosTitulo.textContent =
        "El carrito debe tener al menos un producto para poder finalizar la compra";
...
function crearVerDetalle() {
  ...
  detalle.forEach((videojuego) => {
    ...
    function accion1() {
      imagenPrincipal.src = videojuego.imagen2;
    }
    function accion2() {
      imagenPrincipal.src = videojuego.imagen3;
    }
    function accion3() {
      imagenPrincipal.src = videojuego.imagen4;
    }
  });
}
...
const finalizarCompra = () => {
  ...
  modalInputNumeroTarjeta.setAttribute(
    "placeholder",
    "Ej: 9999-9999-9999-9999"
  );
  ...
  modalInputFechaVencimiento.setAttribute(
    "oninput",
    "InputVencimiento(this.value)"
  );
  ...
  modalBotonFinalizarCompra.addEventListener("click", function formulario() {
    ...
    if (
      modalInputTelefono.value.length <= 0 ||
      isNaN(modalInputTelefono.value)
    ) {
    ...
    if (
      modalInputNumeroTarjeta.value.length !== 16 ||
      isNaN(modalInputNumeroTarjeta.value)
    ) {
    ...
    if (
      modalInputNombre.value.length > 0 &&
      modalInputApellido.value.length > 0 &&
      /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(modalInputMail.value) &&
      modalInputTelefono.value.length > 0 &&
      !isNaN(modalInputTelefono.value) &&
      modalInputNumeroTarjeta.value.length === 16 &&
      !isNaN(modalInputNumeroTarjeta.value) &&
      modalInputFechaVencimiento.value.length === 5 &&
      modalInputCVV.value.length === 3
    ) {
      carrito.splice(0, carrito.length);
      pcantidad.textContent = carrito.length;
      localStorage.clear();
      ...
      const graciasElemento = d.createElement("div"),
        graciasTitulo = d.createElement("h3");
